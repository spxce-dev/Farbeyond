...
export default function ProductCard({ product, index = 0 }) {
  ...
  // ✅ Support BOTH shapes:
  // - proxy list: { id, name, price, image, link }
  // - older UI:   { image_url, original_price, brand, is_new, ... }
  const image =
    product?.image ||
    product?.image_url ||
  ...
  const brand = product?.brand || ""; // proxy doesn't include brand
  ...
  return (
    ...
    >
      {/* ✅ IMPORTANT FIX: pass querystring via React Router object */}
      <Link
        to={{
      ...
      >
        <div className="relative aspect-square bg-neutral-50 rounded-lg overflow-hidden mb-4">
          ...
          {/* NEW badge (only if your product has it) */}
          {product?.is_new ? (
  ...
  onClick={(e) => {
    ...
    const existing = cart.find((i) => Number(i.product_id) === pid);
